...
var appid = "3004884211"
var token = "BnrBK8iyP63Fs7MyplIkHZTEWaJNOaLK"
...
// 一次性合成
func nonStreamSynth(text, voiceType string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("write message fail, err: %w", err)
	}
	_, message, err := c.ReadMessage()
	if err != nil {
		return nil, fmt.Errorf("read message fail, err: %w", err)
	}
	resp, err := parseResponse(message)
	if err != nil {
		return nil, fmt.Errorf("parse response fail, err: %w", err)
	}
...
func Tts(content string) ([]byte, error) {
	return nonStreamSynth(content, "BV421_streaming")
}

func TtsTest() {
	//nonStreamSynth("我想测试下语音合成的效果", "BV421_streaming", "recording.wav")
}
