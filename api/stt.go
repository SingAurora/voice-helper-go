...
func buildAsrClient() AsrClient {
	client := AsrClient{}
	client.Workflow = "audio_in,resample,partition,vad,fe,decode,itn,nlu_ddc,nlu_punctuate"
...
func Stt(file []byte) (string, error) {
	appid := "3004884211"
	token := "BnrBK8iyP63Fs7MyplIkHZTEWaJNOaLK"
	...
	asrResponse, err := client.requestAsr(file)
	fmt.Printf("%+v", asrResponse)
	if err != nil {
		return "", err
	}
...
func SttTest() {
	...
	fileContent, err := os.ReadFile("F:\\MyProject\\GoProject\\voice-helper-go\\local_copy.wav")
	if err != nil {
		fmt.Println("Error reading file:", err)
		return
	}
	asrResponse, err := client.requestAsr(fileContent)
	fmt.Printf("%+v", asrResponse)
	if err != nil {
		fmt.Println(err)
	}
